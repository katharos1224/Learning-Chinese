...
extension HomeViewController: UITableViewDataSource {
    ...
    @objc func recordOurVoice(_ sender: UITapGestureRecognizer) {
        ...
        if isPlayingRecordedVoice {
            searchResultTableView.makeToast("Wait for playing your recorded voice!", duration: 1.0)
        } else {
            if isRecordingVoice {
                voiceRecorder?.stop()
                isRecordingVoice = false
                searchResultTableView.makeToast("Stopped recording your voice!", duration: 1.0)
            } else {
                voiceRecorder?.record()
                isRecordingVoice = true
                searchResultTableView.makeToast("Recording your voice!", duration: 1.0)
            }
        }
        didRecord = true
    }
    ...
    @objc func playOurVoice(_ sender: UITapGestureRecognizer) {
        ...
        if voiceRecorder == nil {
            ...
            if isRecordingVoice {
                ...
                searchResultTableView.makeToast("Wait for recording your voice!", duration: 1.0)
            } else {
                if isPlayingRecordedVoice {
                    voicePlayer?.delegate = self
                    voicePlayer?.stop()
                    isPlayingRecordedVoice = false
                    searchResultTableView.makeToast("Stopped playing your voice!", duration: 1.0)
                } else {
                    setupPlayer()
                    voicePlayer?.play()
                    isPlayingRecordedVoice = true
                    audioPlayer?.delegate = self
                    searchResultTableView.makeToast("Playing your voice!", duration: 1.0)
                }
            }
        }
    }
